...
% grey axis
		\draw[gray!30] (0,0) -- (0,1/\s+1/10);
		\draw[gray!30] (0,0) -- (1/\c+1/10,0);
% arc of the angle
		\draw   (1/5,0) node[sloped,anchor=south east] {$\boldsymbol{\theta}$} arc (0:\angle:1/5) ;

% small triangle (sohcahtoa):
		\draw (0,0) --node [sloped,above]{1} (\angle:1);                % radius = 1
		\draw (0,1*\s) -- node [below]{$\boldsymbol{\cos(\theta)}$} (1*\c,1*\s);     % cos(x)
		\draw (1*\c,1*\s)-- node [right=.25mm]{$\boldsymbol{\sin(\theta)}$}(1*\c,0);     % sin(x)
%circle :
		\draw (1,0) arc(0:90:1);                 % quater circle
% big triangle (shochatao):
		\draw [r] (0   ,0 ) -- node [below] {$\boldsymbol{\sec(\theta)}$} (1/\c,0);            % sec(x)
		\draw [b] (1/\c,0) -- node [sloped,above] {$\boldsymbol{\tan(\theta)}$} (1*\c,1*\s); % tan(x)
		\draw [o] (0  ,  0) -- node [left=1mm] {$\boldsymbol{\csc(\theta)}$} (0,1/\s);            % csc(x)
		\draw [g] (0,1/\s) -- node [sloped,above] {$\boldsymbol{\cot(\theta)}$} (1*\c,1*\s); % cot(x)
	\end{tikzpicture}
}