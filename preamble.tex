...
\NewFactoidBundle{diff std}{%
    ...
    \NewInteramble{\\}
    \NewEntry{diff constant}{\rule{0pt}{4ex}$ c $ & $ 0 $}
    \NewEntry{diff power}{\rule{0pt}{4ex}$ x^n $ & $ n x^{n-1} $}
    \NewEntry{diff exp}{\rule{0pt}{4ex}$ e^x $ & $ e^x $}
    \NewEntry{diff expa}{\rule{0pt}{4ex}$ a^x $ & $ a^x \ln(a) $}
    \NewEntry{diff ln}{\rule{0pt}{4ex}$ \ln(x) $ & $ \frac{1}{x} $}
    \NewEntry{diff loga}{\rule{0pt}{4ex}$ \log_a(x) $ & $ \frac{1}{x \ln(a)} $}
    \NewPostamble{%
      \end{tabular}%
    }
}
...
\NewFactoidBundle{diff inv trigh}{%
  ...
  \NewInteramble{\\}
  \NewEntry{diff arsinh}{\rule{0pt}{4ex}$ \operatorname{arsinh}(x) $ & $ \frac{1}{\sqrt{x^2+1}} $}
  \NewEntry{diff arcosh}{\rule{0pt}{4ex}$ \operatorname{arcosh}(x) $ & $ \frac{1}{\sqrt{x^2-1}} $}
  \NewEntry{diff artanh}{\rule{0pt}{4ex}$ \operatorname{artanh}(x) $ & $ \frac{1}{1-x^2} $}
  \NewEntry{diff arcoth}{\rule{0pt}{4ex}$ \operatorname{arcoth}(x) $ & $ \frac{1}{1-x^2} $}
  \NewEntry{diff arsech}{\rule{0pt}{4ex}$ \operatorname{arsech}(x) $ & $ -\frac{1}{x\sqrt{1-x^2}} $}
  \NewEntry{diff arcsch}{\rule{0pt}{4ex}$ \operatorname{arcsch}(x) $ & $ -\frac{1}{\abs{x}\sqrt{1+x^2}} $}
  \NewPostamble{%
    \end{tabular}%
  }
}
