...
\DeclareFactoid{troll epsilon}{ $1i\rotatebox[origin=c]{180}{w}_{o\leftarrow f}x(\text{f})=\rotatebox[origin=c]{180}{7}$ $\Leftarrow\hspace{-15pt}\raisebox{1pt}{$\implies$}$ \rotatebox[origin=c]{180}{A}$\in\gneq O$, {\Huge\rotatebox[origin=c]{180}{$\in$}}6$\not\leq$O, such\\tHat f\,\rotatebox[origin=c]{-90}{$w$}\,\rlap{b}\rotatebox[origin=c]{-180}{$d$}$\phantom{.}_\delta$($\emptyset$) $\rlap{=}>$ $|\mid \times(f)-\rotatebox[origin=c]{180}{7}||<\epsilon$}
