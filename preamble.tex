...
\DeclareFactoid{geom trig def}{
  ...
  \begin{tikzpicture}[scale=5,very thick]
    ...
    \draw (0,\s) -- node [below]{$\boldsymbol{\cos(\theta)}$} (\c,\s);     % cos(x)
    \draw (\c,\s)-- node [right]{$\boldsymbol{\sin(\theta)}$}(\c,0); % sin(x)
    ...
    \draw [b] (1/\c,0) -- node [sloped,above] {$\boldsymbol{\tan(\theta)}$} (\c,\s);  % tan(x)
    \draw [o] (0  ,  0) -- node [left=1mm] {$\boldsymbol{\csc(\theta)}$} (0,1/\s);        % csc(x)
    \draw [g] (0,1/\s) -- node [sloped,above] {$\boldsymbol{\cot(\theta)}$} (\c,\s);  % cot(x)
  \end{tikzpicture}
}